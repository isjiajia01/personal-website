...
type StickerId = "github" | "docker" | "cloudflare" | "codex" | "claude" | "typescript" | "swift";
...
interface StickerDefinition {
   ...
   *  - "silhouette"   — paint-order stroke expansion of the icon path (e.g. Docker, Cloudflare, Codex, Claude)
   */
...
const STICKER_STORAGE_KEY = "__printer_sticker_layout_v6__";
const STICKER_ORDER_STORAGE_KEY = "__printer_sticker_order_v2__";
 ...
 *  - "silhouette"   — paint-order stroke expansion of the path (Docker, Cloudflare, Codex, Claude).
 */
function renderStickerFace(sticker: StickerDefinition, resolvedMode: ResolvedColorMode) {
  ...
  const pathFillRule = sticker.fillRule ?? "nonzero";
  ...
  const pathTransform = `translate(${iconOffsetX} ${iconOffsetY}) ${iconTransform}`;
  ...
  if (sticker.bgShape === "circle") {
    ...
    // "silhouette" — paint-order expansion around organic logo shapes.
    bgElement = (
    ...
    >
      ...
      <g transform={`translate(${iconOffsetX} ${iconOffsetY})`}>
        <g transform={iconTransform}>
          <path d={sticker.svgPath} fill={iconColor} fillRule={pathFillRule} clipRule={pathFillRule} />
        </g>
      </g>
    </svg>
  );
